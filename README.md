...
1. **Register domain** (~CHF 15/yr, e.g. Infomaniak). The pages assume `deklara.ch` — if you register something else, search-and-replace `deklara.ch` in `index.html`, `fr/index.html`, `robots.txt`, `sitemap.xml`.
2. **Create a Formspree form** (formspree.io, free tier is fine) and replace `DEIN_FORM_ID` in the `FORM_ENDPOINT` constant at the bottom of **both** `index.html` and `fr/index.html`. Until then the page runs in *fake-door mode*: it shows the success message locally and only fires analytics (no email is stored!).
3. **Set up Plausible Analytics** (plausible.io) for your domain. The tracking snippet is already in the `<head>` of both pages (`data-domain="deklara.ch"` — update if your domain differs). Goals to create in Plausible: `Signup`, `PreorderClick`, `TierChoice` (custom events, already wired with props: `tier`, `cleanup`, `lang`).
