...
1. **Register domain** (~CHF 15/yr, e.g. Infomaniak). The pages assume `deklar.ch` — if you register something else, search-and-replace `deklar.ch` in `index.html`, `fr/index.html`, `en/index.html`, `robots.txt`, `sitemap.xml`.
...
3. **Set up Plausible Analytics** (plausible.io) for your domain. The tracking snippet is already in the `<head>` of both pages (`data-domain="deklar.ch"` — update if your domain differs). Goals to create in Plausible: `Signup`, `PreorderClick`, `TierChoice` (custom events, already wired with props: `tier`, `cleanup`, `lang`).
